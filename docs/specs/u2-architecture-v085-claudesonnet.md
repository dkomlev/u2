...
* **Базовая частота:** 15 Hz (снапшоты от сервера к клиентам)
...
* Снижение нагрузки на сервер (~5% CPU)
...
#### **5.3.1. Обновление тепловой модели**

Тепловая модель обновляется с **пониженной частотой (15 Hz)** для экономии CPU:

1. **На каждом физическом тике (30 Hz):**
   - Накапливается `heat_delta` от работы двигателей и выстрелов
   - НЕ выполняется интеграция и проверка порогов

2. **На каждом 2-м тике (15 Hz):**
   - Применяется накопленный `heat_delta`
   - Рассчитывается охлаждение
   - Обновляется температура: `T' = T + (heat_delta - cooling) × (2×dt)`
   - Проверяются пороги троттлинга

3. **Между обновлениями:**
   - Температура интерполируется линейно для HUD
   - Троттлинг применяется на основе последнего известного значения

Это допущение корректно, так как тепловые процессы медленные (секунды), и задержка 33 мс несущественна.
...
* **Сигнатура CS:** 1 (stealth) – 5 (capital)

**Типичные корабли для M0-M8:**
* **small fighter** (55 т, 80 м/с², CS=2) — базовый истребитель
* **small freighter** (80 т, 35 м/с², CS=3) — грузовой
* **medium gunship** (180 т, 40 м/с², CS=3) — боевой с экипажем
...
**1. Щиты (Shields):**
* Первая линия защиты
* Автоматическая регенерация (2% HP/сек после 3-5 сек без урона)
* Разделены на faces (4-6 в зависимости от размера корабля)
* Формула: `shield_HP = hull_HP × 1.5`

**2. Броня (Armor):**
* Пассивное снижение урона в процентах
* Зависит от типа корабля (5-35%)
* НЕ восстанавливается автоматически

**3. Корпус (Hull HP):**
* Основные очки прочности
* Формула: `hull_HP = mass_t × 100 × type_multiplier`
* Восстановление только при докинге/ремонте

**Type_multiplier по типу:**
* sport/interceptor: 0.8
* fighter: 1.0
* courier: 1.1
* freighter: 1.3
* gunship/warship: 1.5
...
**Типичные значения (примеры):**
* small fighter (55 т): 5,500 hull HP, 8,250 shield HP, 10% armor
* medium gunship (180 т): 27,000 hull HP, 40,500 shield HP, 35% armor

**Механика урона:**
1. Урон сначала применяется к активной shield face
2. Если face = 0, урон проходит к корпусу
3. Броня снижает урон: `actual_damage = damage × (1 - armor%)`
4. Hull HP уменьшается

При Hull HP ≤ 0: корабль уничтожен → wreck (обломки)

**Особенности щитов:**
* **Energy weapons:** полностью поглощаются щитами
* **Ballistic weapons:** 30% урона проходит через щит к корпусу
* **Face reinforcement:** игрок может перераспределить до 50% силы щита на одну грань (в будущих версиях)

**Сенсоры:**
* Радиус обнаружения: 10-50 км (зависит от корабля)
...
### **6.3. Распределение тяги по двигателям (Thrust Allocation)**

Корабль может иметь **несколько двигателей** в разных точках корпуса. Задача системы распределения тяги — преобразовать желаемую силу и момент в конкретные уровни тяги для каждого двигателя.

**Входы:**
* Желаемая сила: `F_desired` (вектор)
* Желаемый момент: `M_desired` (скаляр в 2D, вектор в 3D)

**Выходы:**
* Уровень тяги каждого двигателя: `thrust[i]` ∈ [0, max_thrust[i]]

**Алгоритм (упрощённый):**

1. **Линейная система:** найти `thrust[]` такие, что:
   - `Σ(thrust[i] × direction[i]) = F_desired`
   - `Σ(thrust[i] × lever_arm[i]) = M_desired`

2. **Ограничения:**
   - `0 ≤ thrust[i] ≤ max_thrust[i]`
   - Минимизировать расход топлива (в будущем)

3. **Решение:** квадратичное программирование или жадный алгоритм

**Для минимальной версии:**
Большинство кораблей имеют **симметричные наборы двигателей**, что упрощает распределение до простых формул.

**Пример: истребитель с 4 двигателями**
```
     [Нос]
      ↑
  [L1] [R1]  ← передние
...
- Вперёд: все 4 двигателя поровну
- Поворот влево: L1, L2 сильнее, чем R1, R2
- Влево без вращения: L1+R2 vs L2+R1
```

**Важно:** Thrust Allocation реализуется **ПОСЛЕ M5**, в рамках M7. В M0-M5 используется **упрощённая модель** с одним "виртуальным двигателем".
...
**Режим Coupled (управляемый занос)** выносится за рамки минимальной версии и планируется для M9+ как "advanced flight model".
