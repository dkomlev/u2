...
Версия: 0.8.5 (ChatGPT)\
Дата: 2025‑11‑15

Документ описывает план разработки с участием ИИ‑агентов для прототипа космического симулятора **Universe Unlimited**. Версия 0.8.5 учитывает:

- актуальную архитектуру (U2 — Архитектура 0.8.3+);
- вынесение тактико‑технических характеристик кораблей в отдельный документ **«U2 — ТТХ кораблей v0.8»**;
- рекомендации по упрощению и удешевлению реализации (Документ 5: «Рекомендации по правкам Плана разработки», версия 0.8.5 ChatGPT);
...
1. Источники команд управления:
   ...
   - сценарные и отладочные скрипты (для ботов и тестовых сцен).
...
6. **Арбитраж источников управления**
   - Приоритет: Local Input > Remote Operator > Autopilot > сценарии.
...
5. **Постоянная скорость света на локацию.** В M0–M8 не реализуется поле скорости света c′(x), зависящее от координат или нагрузки.
...
**Автопилот** — программа, которая самостоятельно управляет кораблём по заданным целям (маршруты, стандартные манёвры).

**Авторитетный сервер** — сервер, который хранит истинное состояние мира и имеет приоритет над клиентскими расчётами.

**Время пролёта света** — время, за которое свет проходит расстояние от объекта до наблюдателя. В игре вычисляется как расстояние, делённое на c′.

**Инвариант** — условие, которое всегда должно выполняться (например, ограничение скорости корабля сверху долей от c′).

**Компонентная система (ECS)** — способ описания объектов через набор компонент, каждая отвечает за один аспект (положение, физика, управление).

**Коэффициент γ (гамма)** — множитель, отражающий релятивистские эффекты при скоростях, близких к скорости света.

**Скорость света c′** — эффективная скорость распространения сигналов в игровом мире, заниженная по сравнению с реальной и постоянная для каждой локации.

**Очередь будущих состояний** — буфер на клиенте, в котором хранятся состояния объектов с учётом задержки связи и времени пролёта света, для плавного отображения.

**Панель приборов (HUD)** — часть интерфейса, где пилот видит ключевые параметры корабля и окружения.

**Reconciliation** — корректировка клиентского предсказания состояния по данным сервера.

**Snapshot (снимок состояния)** — набор данных, описывающий состояние мира в определённый момент времени, отправляемый сервером клиенту.

**RTT (Round‑Trip Time)** — время, за которое сетевой пакет проходит от клиента к серверу и обратно.

**Jitter** — изменчивость RTT; показывает, насколько нестабильны сетевые задержки.

**Thrust (тяга)** — суммарное усилие, создаваемое двигателями корабля.

**Signature (сигнатура)** — заметность корабля для средств обнаружения и наведения.

**Lock‑target (захват цели)** — выбор объекта в качестве цели для прицеливания и стрельбы.

**Wreck (обломки)** — состояние корабля после разрушения корпуса, когда он больше не управляется, но остаётся объектом в мире.

**Shield Face (грань щита)** — направленная часть защитного поля щита.

**Hull HP (прочность корпуса)** — основной запас прочности корпуса корабля.

**Armor (броня)** — защита, снижающая входящий урон на определённый процент.
